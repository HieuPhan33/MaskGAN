...
#         noisy = (image + gauss).clip(0,1).astype(np.uint8)
#         noisy = Image.fromarray(255*noisy)
#         return noisy
...
class GaussianNoise:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        z_score = -st.norm.ppf(self.percent/2)
        mask = mask.float()
        gauss = torch.randn_like(mask)
        binary_gauss = 255*(gauss > z_score)
        binary_gauss[gauss < -2*z_score] = -255
        out = mask + binary_gauss
...
class RandomMaskAffine:
    def __init__(self, angle=0, translate=[0, 0], shear=[0, 0]) -> None:
        self.angle = angle
        self.translate = translate
        self.shear = shear
        self.seg_fill = 0
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        return img, TF.affine(mask, self.angle, self.translate, self.scale, self.shear, TF.InterpolationMode.NEAREST, self.seg_fill)
...
class BottomCrop:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        H, W = img.shape[1:]
        img = img[:, H-self.size:, W//2 - self.size//2: W//2 + self.size//2]
        mask = mask[:, (H - self.size):, W//2 - self.size//2: W//2 + self.size//2]
        assert img.shape == mask.shape, f"Mismatch {img.shape}, {mask.shape}"
        return img, mask
...
class Pad:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        pad_h = (self.size[0] - img.shape[1])//2
        pad_w = (self.size[1] - img.shape[2])//2
...
class ResizePad:
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # pad the image
        padding = [(tW - nW)//2, (tH - nH)//2, (tW - nW)//2, (tH-nH)//2]
